...
exports.verifyPayment = function(payment, cb) {
        ...
        function checkReceipt(error, result, environment) {
            ...
            var productQty = {};
            if (receipt.in_app) {
                receipt.in_app.forEach(function(order) {
                    productQty[order.product_id] = productQty[order.product_id] || 0;
                    productQty[order.product_id] += Number(order.quantity);
                });
            }

            if (productQty.hasOwnProperty('in_app')

                if (payment.hasOwnProperty('productId') && !productQty.hasOwnProperty(payment.productId)) {
                    return cb(new Error('Wrong product ID: ' + payment.productId + ' (expected: ' + receipt.in_app + ')'));
                }
